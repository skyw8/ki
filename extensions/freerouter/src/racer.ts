...
export function mergeSignals(parent, local) {
  if (!parent) return { signal: local, cleanup: () => {} };
  if (parent.aborted) return { signal: parent, cleanup: () => {} };
  if (local.aborted) return { signal: local, cleanup: () => {} };
 ...
 */
export async function runFreeRouterStream({
  request,
  config,
  pool,
  apiKey,
  ...
  if (!apiKey) {
    emit({
      ...
      reason: "error",
      error:
        "No OpenRouter API key configured. Set it via PUT /v1/providers/free-router/credential, the extension config apiKey, or OPENROUTER_API_KEY.",
    });
    return;
  }
  ...
  for (let batch = 1; batch <= config.maxBatches; batch++) {
    ...
    if (outcome.winner) return; // raceBatch already emitted done
    if (outcome.stalled || outcome.winnerStreamError) return; // terminal error already emitted
...
async function raceBatch({ candidates, request, apiKey, baseUrl, config, signal, emit, offset, emitThinking, getThinkingText, fetchImpl, log }) {
  ...
  candidates.forEach((modelId, i) => {
    ...
    streamFreeModel({
      ...
      if (err instanceof ModelExhaustedError) exhausted.push(modelId);
  ...
  const remap = (event) =>
    event.contentIndex === undefined ? event : { ...event, contentIndex: event.contentIndex + offset };
  ...
  const finishWithWinner = async (idx, qualifyingEvent) => {
    ...
    while (true) {
      ...
      if (done) return { winner: candidates[idx], exhausted, fatal };
      if (event.type === "done") {
  ...
  try {
    ...
    while (pending.size > 0) {
      ...
      pending.delete(idx);
      if (result.done) continue; // candidate stream ended without qualifying
      const event = result.value;

      if (QUALIFYING.has(event.type)) {
        return await finishWithWinner(idx, event);
      }
      if (event.type === "error") continue; // stream ends next tick; floating catch records cooldown
