...
// CompletionsBody encodes an OpenAI Chat Completions request.
func CompletionsBody(req Request) map[string]any {
	msgs := []map[string]any{}
	if req.System != "" {
	...
	//
	// Encode consecutive toolResults from one parallel batch as role:tool
	// messages first, using plain text only; collect that batch's images in one
	// user message after the tool group. Keep images from earlier turns after
	// their original group rather than moving them to the end of the conversation
...
func toOpenAIMessage(m Message) map[string]any {
	switch m.Role {
	case "toolResult":
		text := m.Text()
		if text == "" && hasImage(m) {
...
func openAIContentParts(m Message) []map[string]any {
...
func openAIImageURLs(m Message) []map[string]any {
	var imgs []map[string]any
...
func parseCompletionsSSE(_, data string, acc *Message) sseParseResult {
	var chunk struct {
		ID      string `json:"id"`
		Choices []struct {
			Delta struct {
	...
	if chunk.Usage != nil {
		...
		acc.Usage = &Usage{
			...
			// Keep the provider's counters verbatim; the caller can normalize them
			// when applying its own pricing model.
	...
	if d.Content != "" {
		...
		deltas = append(deltas, AssistantDelta{Type: "text_delta", Delta: d.Content, Partial: *acc})
	}
	if d.Refusal != "" {
		appendText(acc, d.Refusal)
		deltas = append(deltas, AssistantDelta{Type: "text_delta", Delta: d.Refusal, Partial: *acc})
	}
	if d.ReasoningContent != "" {
		appendThinking(acc, d.ReasoningContent)
		deltas = append(deltas, AssistantDelta{Type: "thinking_delta", Delta: d.ReasoningContent, Partial: *acc})
	}
	for _, tc := range d.ToolCalls {
		...
		if call != nil && tc.Function.Arguments != "" {
			deltas = append(deltas, AssistantDelta{Type: "toolcall_delta", Delta: tc.Function.Arguments, ToolCallID: call.ID, ToolName: call.Name, Partial: *acc})
		}
	}
	if d.FunctionCall != nil {
		...
		if call != nil && d.FunctionCall.Arguments != "" {
			deltas = append(deltas, AssistantDelta{Type: "toolcall_delta", Delta: d.FunctionCall.Arguments, ToolCallID: call.ID, ToolName: call.Name, Partial: *acc})
		}
	}
...
func completionToolCallAt(m *Message, index int) *Content {
	if index < 0 {
		return nil
	}
	position := 0
