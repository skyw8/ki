...
function DeepWebSearchConfigForm({ api, name, onClose, embedded = false, models = [] }: ExtensionConfigEditorProps) {
  const { t, lang } = useI18n()
	...
	const [codexModel, setCodexModel] = useState('gpt-6-astra')
	...
	const [summaryModel, setSummaryModel] = useState('openai-codex/gpt-6-astra')
	...
	const apply = (next: Record<string, unknown>, metadata?: ExtensionConfig) => {
		...
		const rawCodexModel = typeof next.codexModel === 'string' ? next.codexModel : 'gpt-6-astra'
		...
		setSummaryModel(typeof next.summaryModel === 'string' ? next.summaryModel : 'openai-codex/gpt-6-astra')
