...
func parseAnthropicSSE(event, data string, acc *types.Message) sseParseResult {
	...
	case "content_block_delta":
		...
		case "thinking_delta":
			...
			return sseParseResult{delta: loop.AssistantDelta{Type: "thinking_delta", Delta: thinking, Partial: *acc}, emit: thinking != ""}
	...
	case "content_block_start":
		...
		case "text":
			...
			if block.Text != "" {
				return sseParseResult{delta: loop.AssistantDelta{Type: "text_delta", Delta: block.Text, Partial: *acc}, emit: true}
			}
		case "thinking":
			...
			if block.Thinking != "" {
				return sseParseResult{delta: loop.AssistantDelta{Type: "thinking_delta", Delta: block.Thinking, Partial: *acc}, emit: true}
			}
		...
		case "tool_use":
			...
			if len(value) > 0 {
				...
				return sseParseResult{delta: loop.AssistantDelta{
...
func anthropicContentBlockAt(m *types.Message, index int) *types.Content {
...
func anthropicExistingContentBlock(m *types.Message, index int, typ string) (*types.Content, error) {
...
func anthropicContentBlock(m *types.Message, index int, typ string) (*types.Content, error) {
	...
	block := types.Content{Type: typ, StreamIndex: index}
...
func mergeAnthropicUsage(acc *types.Message, usage map[string]any) {
	if acc.Usage == nil {
...
func applyAnthropicStopReason(acc *types.Message, reason string) {
	switch reason {
	case "tool_use":
	...
	case "end_turn", "stop_sequence":
		acc.StopReason = "stop"
	case "pause_turn":
		// Ki has no resumable pause state; preserve the completed stream as a
		// normal stop rather than retrying a request that may have side effects.
		acc.StopReason = "stop"
...
func validateAnthropicRequest(req loop.Request) error {
			...
			case "toolCall":
				...
				if block.Arguments == nil {
					...
					if validObjectArgumentsRaw(block.ArgumentsRaw) == "" {
						...
						// Anthropic input, so toAnthropicMessage will send {}.
...
func validateAnthropicOutput(acc *types.Message) error {
