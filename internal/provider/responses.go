...
func parseResponsesSSE(event, data string, acc *types.Message) sseParseResult {
	...
	case "response.output_text.delta":
		...
		return sseParseResult{delta: loop.AssistantDelta{Type: "text_delta", Delta: text, Partial: *acc}, emit: text != ""}
	case "response.output_text.done":
		...
		return sseParseResult{delta: loop.AssistantDelta{Type: "text_delta", Delta: delta, Partial: *acc}, emit: delta != ""}
	...
	case "response.reasoning_summary_part.added":
		...
		return sseParseResult{delta: loop.AssistantDelta{Type: "thinking_delta", Delta: text, Partial: *acc}, emit: text != ""}
	...
	case "response.function_call_arguments.delta":
		...
		return sseParseResult{delta: loop.AssistantDelta{Type: "toolcall_delta", Delta: delta, ToolCallID: c.ID, ToolName: c.Name, Partial: *acc}, emit: delta != ""}
	...
	case "response.custom_tool_call_input.delta":
		...
		c.ToolType = "custom"
		c.Input += delta
		return sseParseResult{delta: loop.AssistantDelta{Type: "custom_tool_call_input_delta", Delta: delta, ToolCallID: c.ID, ToolName: c.Name, Partial: *acc}, emit: delta != ""}
...
func setResponseID(acc *types.Message, obj map[string]any) {
	response := responseObject(obj)
...
func applyResponsesOutputItem(acc *types.Message, item map[string]any) sseParseResult {
	...
	case "message":
		...
		return sseParseResult{delta: loop.AssistantDelta{Type: "text_delta", Delta: textDelta, Partial: *acc}, emit: textDelta != ""}
...
func responseTextBlock(acc *types.Message, itemID string) *types.Content {
	...
	acc.Content = append(acc.Content, types.Content{Type: "text", ItemID: itemID})
	return &acc.Content[len(acc.Content)-1]
}

func appendResponseText(acc *types.Message, itemID, delta string) {
	if delta == "" {
		return
	}
...
func mergeResponseText(acc *types.Message, itemID, text string) string {
	if text == "" {
		return ""
	}
	block := responseTextBlock(acc, itemID)
...
func responseThinkingBlock(acc *types.Message, itemID string) *types.Content {
	for i := range acc.Content {
	...
	acc.Content = append(acc.Content, types.Content{Type: "thinking", ItemID: itemID})
	return &acc.Content[len(acc.Content)-1]
}

func appendResponseThinking(acc *types.Message, itemID, delta string) {
	if delta == "" {
		return
	}
...
func mergeResponseThinking(block *types.Content, text string) {
	if block == nil || text == "" {
		return
	}
...
func setResponseFunctionArguments(c *types.Content, raw string) error {
	if c == nil {
...
func applyResponsesUsage(acc *types.Message, response map[string]any) {
