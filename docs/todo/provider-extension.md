...
- **传输**：由扩展内独立的 `openai-codex-responses` streamer 完成，不复用 ki core 的普通 OpenAI Responses。首版先实现 SSE：`https://chatgpt.com/backend-api/codex/responses`、Codex headers、`store:false`、`include: reasoning.encrypted_content`、Responses message/function/custom-tool 事件和 usage/stop reason 映射；WS 连接复用、增量上下文和压缩作为后续优化。扩展向 ki 只回传 delta RPC 事件。
...
- fake 单测覆盖 PKCE 参数、account ID、refresh、请求体、Codex headers、SSE 文本和 function call。真实 OAuth/Codex 账号验证仍需手工执行；WebSocket/zstd/增量 `previous_response_id` 不在首版验收范围。
