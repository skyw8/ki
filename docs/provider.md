...
- Responses 文本消息兼容少数网关缺失 `message.id` 的返回：优先用 `item_id`，其次用 `output_index` 或唯一未关闭文本项关联；function call、custom tool 和 reasoning 仍要求有效 item ID，以保证无状态回放。确定性的请求/协议错误（普通 4xx、缺失终止事件、非法 SSE）不重试；网络错误、408/409/425/429、5xx，以及兼容网关明确返回的瞬时 `bad_response_status_code` 才进入 loop 的退避重试。
