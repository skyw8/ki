...
- Info：本会话只读元数据、skills、MCP（含已缓存工具）、slash 命令；内容右侧提供 sticky outline，除区块外还列出具体 skill、MCP server/tool 和 slash command，可快速跳转并随滚动高亮。MCP 按 server 展示来源、启用状态、连接地址和工具卡片；Reload 清进程缓存（按钮有加载动画和完成提示），Edit 打开设置。不在此页开关。
