...
- 对话页只渲染服务端返回的最新窗口（默认 100 条 leaf entries），更早的历史通过 `?before=` 在滚到顶部时按页补入；整棵树的 `index` 由后台单独请求（`fields=index`，比一次尾部读取还短的会话随默认响应一起返回），到达后只补齐分支导航、轨迹表和绝对轮次编号，不增加聊天节点。轮次编号在 index 未到达前是相对窗口的。打开会话因此只取决于最新一页，不随历史大小增长；向上翻页沿用 `loadOlder`（距顶 96px 触发，`olderLock` 串行化）。
