...
- 扩展壳（不加载扩展 JS）：session GET 的 `extensionUi` + SSE `extension_ui_updated` / `extension_notice`。顶栏 StatusChip（短文本 + tone），点击打开 DetailDrawer（panel 的 markdown/kv、actions、可编辑 fields）。`ui.action` / `ui.submit` 回 sidecar。`ui.confirm` / `ui.select` 弹层（`extensionUi[].prompt`），超时 120s 或 abort 视为取消。status/panel 不进 jsonl；Reload 后由 sidecar 按 custom entry 重放。slash 面板消费 `commands[].argumentHint` / `completions`。扩展 enqueue 的 user 气泡带 `origin`。
- Info：本会话只读元数据、skills、MCP（含已缓存工具）、extensions、slash 命令；内容右侧提供 sticky outline。`path` 只展示字符串，不当 `href`。Reload 清资源快照并关闭 MCP 与 extension sidecar。Edit 打开设置。不在此页开关。
