...
| MCP | `mcp_server_failed`、`mcp_tools_changed` | MCP 失败，或工具目录需要 Reload。 |
| 扩展 UI/状态 | `extension_error`、`extension_notice`、`extension_ui_prompt` | 扩展失败、toast，或 WebUI 确认/选择弹层。 |
| Runtime | `runtime_ready` | session 打开时的扩展/MCP 准备结束；成功或失败都会解锁 session。 |
...
| 仅 async | `after_provider_response`、`agent_start`、`agent_end`、`agent_settled`、`turn_start`、`turn_end`、`message_start`、`request_header`、`tool_execution_start`、`tool_execution_end`、`compaction_start`、`compaction_end`、`queue_changed`、`steer_accepted`、`run_aborted`、`mcp_server_failed`、`mcp_tools_changed` |

`provider_error` 的 sync 订阅只有扩展在 `initialize` 中声明 `fallback` 时才
能介入错误处理。

`message_update`、`tool_execution_update`、`context_usage`、
...
Server -> MCP: Prepare / discover
MCP --> Server: server_failed | tools_changed
Server -> SSE: mcp_server_failed / mcp_tools_changed
...
par 队列和控制
  ...
  MCP -> Server: failure / tools_changed
  Server -> SSE: mcp_server_failed / mcp_tools_changed
else 扩展 UI
