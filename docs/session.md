...
- `GET /v1/sessions/{id}` 带只读 `availableSkills` / `availableMcp`（含已缓存 MCP tools）和 `commands[]`。`enabled` 来自全局 `toggles.json`。列举 MCP 不 spawn。Prompt 用 serve 级池的缓存 schema 组 tools，真 call 才连。
