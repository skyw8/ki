...
- 目录内：`events.jsonl` + `config.json`；忙时排队的 user 在 `queue.json`（最多 100 条 FIFO，不进消息树直到出队开跑）；不触发运行但要进入后续 prompt 的正常 user message 暂存于 `context-queue.json`（最多 100 条，按序提交）。`config.json` / `queue.json` / `ext-queue.json` / `context-queue.json` 经同目录临时文件 + rename 原子落盘，避免并发读到截断 JSON。
